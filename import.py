...
def main():
    ...
    for isbn, title, author, year in reader:
        # Insert data (: is placeholder).
        db.execute("INSERT INTO cs50_books (isbn, title, author, year) VALUES (:isbn, :title, :author, :year)", {
                   "isbn": isbn, "title": title, "author": author, "year": year})
        counter = counter + 1

        # Print when entry added to database.
        print(f"Counter = {counter}. {title} ({year}), {author}. ISBN: {isbn}. Successfully added to database!")

    # Save changes.
    db.commit()

    print(f"{counter} books added to database table 'cs50_books'.")
